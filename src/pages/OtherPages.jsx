...
// ─── Notice content renderer — links clickable, YouTube thumbnail ───
function NoticeContent({ content }) {
  if (!content) return null;
  // URL detect করার regex
  const urlRegex = /(https?:\/\/[^\s]+)/g;
  const parts = content.split(urlRegex);
  return (
    <div className="text-gray-400 text-sm leading-relaxed space-y-3">
      {/* Text parts with clickable links */}
      <p className="whitespace-pre-wrap">
        {parts.map((part, i) => {
          if (!part.match(urlRegex)) return <span key={i}>{part}</span>;
          const ytId = getYouTubeId(part);
          if (ytId) {
            // YouTube link — just make it clickable here, thumbnail shown below
            return (
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card p-8 text-center flex flex-col items-center gap-4">
            ...
            {/* Agrodut Facebook Button */}
            {settings.facebook_url && (
              <a href={settings.facebook_url} target="_blank" rel="noreferrer"
                className="inline-flex items-center gap-3 px-6 py-3 rounded-lg font-semibold transition-all text-white w-full max-w-sm justify-center" style={{ background: "#1877f2" }}>
                <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
                Follow Agrodut on Facebook
              </a>
            )}

            {/* JZSSC Facebook Button */}
            <a href="https://www.facebook.com/share/1GLCyhA1Gn/" target="_blank" rel="noreferrer"
              className="inline-flex items-center gap-3 px-6 py-3 rounded-lg font-semibold transition-all text-white w-full max-w-sm justify-center" style={{ background: "#1877f2" }}>
              <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
              Follow JZSSC on Facebook
            </a>
          </div>
        </div>
      </div>
