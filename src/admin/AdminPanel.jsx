...
async function generateCardsPDF(cards, type, siteUrl) {
  ...
  const cardH=(297-margin*5)/4;
  const typeLabels = { quiz:"Quiz", olympiad:"Olympiad", science_project:"Science Project" };
  ...
  while (idx<cards.length) {
    ...
    for (let row=0;row<4;row++) {
      for (let col=0;col<2;col++) {
        ...
        doc.setFont("helvetica","bold"); doc.setFontSize(7); doc.setTextColor(0,220,240);
        doc.text(`SPARK FEST — ${typeLabel.toUpperCase()} CARD`,x+cardW/2,y+6.5,{align:"center"});
  ...
  doc.save(`sparkfest-${type}-cards.pdf`);
}
...
async function generateRegListPDF(data, title) {
  ...
  doc.text("#",14,y); doc.text("Name/Team",24,y); doc.text("Institution",100,y); doc.text("Status",170,y);
  y+=8;
  data.forEach((r,i)=>{
    ...
    doc.text((r.team_name||r.name||"").substring(0,30),24,y);
    doc.text((r.institution||"").substring(0,25),100,y);
    const st=r.payment_status||"registered";
...
async function generateResultPDF(data, title) {
  ...
  doc.setFont("helvetica","italic"); doc.setFontSize(7); doc.setTextColor(150,150,150);
  doc.text(`Spark Fest 2026 | ${new Date().toLocaleString()}`,pageW/2,285,{align:"center"});
...
const COMP_TYPES=[
  {id:"quiz",            label:"Quiz",                    icon:"🧩", color:"#00f5ff"},
  {id:"olympiad",        label:"Olympiad",                icon:"🔭", color:"#a855f7"},
  {id:"science_project", label:"Science Project",         icon:"🔬", color:"#f59e0b"},
  {id:"astro_photo",     label:"Astro Photography",       icon:"🌌", color:"#06b6d4"},
];
...
function Dashboard({setTab}) {
  ...
  useEffect(()=>{
    async function load(){
      try{
        const [q,o,sp,ap]=await Promise.all([
          ...
          apiPost({action:"getRegistrations",type:"science_project",token:admin.token}),
          apiPost({action:"getRegistrations",type:"astro_photo",token:admin.token}),
        ]);
        const qd=q.data||[]; const od=o.data||[]; const spd=sp.data||[]; const apd=ap.data||[];
        setStats({
          quiz:qd.length, olympiad:od.length, science_project:spd.length, astro_photo:apd.length,
          totalReg:qd.length+od.length+spd.length+apd.length,
          pending:qd.filter(r=>r.payment_status==="pending").length+od.filter(r=>r.payment_status==="pending").length+spd.filter(r=>r.payment_status==="pending").length,
          confirmed:qd.filter(r=>r.payment_status==="confirmed").length+od.filter(r=>r.payment_status==="confirmed").length+spd.filter(r=>r.payment_status==="confirmed").length+apd.length,
        });
        const all=[
          ...qd.map(r=>({...r,_type:"Quiz",_color:"#00f5ff"})),
          ...od.map(r=>({...r,_type:"Olympiad",_color:"#a855f7"})),
          ...spd.map(r=>({...r,_type:"Science Project",_color:"#f59e0b"})),
          ...apd.map(r=>({...r,_type:"Astro Photo",_color:"#06b6d4",payment_status:"registered"})),
        ].sort((a,b)=>new Date(b.registered_at)-new Date(a.registered_at)).slice(0,6);
        setRecentRegs(all);
      }catch{}
    }
    load();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-6 relative overflow-hidden"
        ...
        <p className="text-gray-400 text-sm">{new Date().toLocaleDateString("en-GB",{weekday:"long",day:"numeric",month:"long",year:"numeric"})}</p>
        <div className="mt-4 flex flex-wrap gap-2">
          {[
            {label:`⚡ ${stats?.totalReg??0} Total Registrations`,color:"#00f5ff"},
            {label:`⏳ ${stats?.pending??0} Pending`,color:"#f59e0b"},
            {label:`✅ ${stats?.confirmed??0} Confirmed`,color:"#22c55e"},
          ].map((b,i)=>(
            <span key={i} className="text-xs px-3 py-1.5 rounded-full font-medium"
              style={{background:b.color+"22",color:b.color,border:`1px solid ${b.color}44`}}>{b.label}</span>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          {icon:"🧩",label:"Quiz",          value:stats?.quiz,           color:"#00f5ff"},
          {icon:"🔭",label:"Olympiad",      value:stats?.olympiad,       color:"#a855f7"},
          {icon:"🔬",label:"Science Project",value:stats?.science_project,color:"#f59e0b"},
          {icon:"🌌",label:"Astro Photo",   value:stats?.astro_photo,    color:"#06b6d4"},
        ].map((s,i)=>(
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <div className="glass-card p-5 md:col-span-1">
          <h3 className="font-bold text-white mb-4 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-cyan-400 animate-pulse inline-block"/>Quick Actions
          </h3>
          <div className="space-y-2">
            {[
              {label:"Generate Cards",    tab:"cards",   icon:"🎴",color:"#00f5ff"},
              {label:"Registrations",     tab:"regs",    icon:"📋",color:"#a855f7"},
              {label:"Exam Marks",        tab:"marks",   icon:"📝",color:"#f59e0b"},
              {label:"Results",           tab:"results", icon:"🏆",color:"#22c55e"},
              ...
              <button key={a.tab} onClick={()=>setTab(a.tab)}
                ...
                <span className="w-7 h-7 rounded-lg flex items-center justify-center text-sm shrink-0"
                  style={{background:a.color+"18",border:`1px solid ${a.color}44`}}>{a.icon}</span>
        ...
        <div className="glass-card p-5 md:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-white flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-purple-400 animate-pulse inline-block"/>Recent Registrations
            </h3>
            <button onClick={()=>setTab("regs")} className="text-cyan-400 text-xs hover:underline">View all →</button>
          </div>
          {recentRegs.length===0?(
            <div className="text-center py-8 text-gray-600"><p className="text-3xl mb-2">📋</p><p className="text-sm">No registrations yet</p></div>
          ):(
            <div className="space-y-2">
              {recentRegs.map((r,i)=>(
                <div key={i} className="flex items-center gap-3 px-3 py-2.5 rounded-lg" style={{background:"rgba(255,255,255,0.03)"}}>
                  <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm shrink-0"
                    style={{background:r._color+"22"}}>{COMP_TYPES.find(c=>c.label.toLowerCase().includes(r._type.toLowerCase().split(" ")[0]))?.icon||"📋"}</div>
                  <div className="flex-1 min-w-0">
                    <p className="text-white text-sm font-medium truncate">{r.team_name||r.name}</p>
                    <p className="text-gray-500 text-xs">{r.institution} · <span style={{color:r._color}}>{r._type}</span></p>
                  </div>
                  <div className="text-right shrink-0">
                    <span className="text-xs px-2 py-0.5 rounded-full font-medium"
                      style={{
                        background:r.payment_status==="confirmed"||r.payment_status==="registered"?"rgba(34,197,94,0.15)":r.payment_status==="rejected"?"rgba(239,68,68,0.15)":"rgba(245,158,11,0.15)",
                        color:r.payment_status==="confirmed"||r.payment_status==="registered"?"#22c55e":r.payment_status==="rejected"?"#ef4444":"#f59e0b"
                      }}>
      ...
      <div className="glass-card p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-white flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-yellow-400 animate-pulse inline-block"/>Active Notices ({notices.length})
          </h3>
...
function CardGenerator() {
  ...
  const CARD_TYPES=[
    ...
    {id:"science_project", label:"Science Project"},
  ];

  async function loadBatches(){
    setFetching(true);
    try{const d=await apiPost({action:"getCardBatches",token:admin.token});if(d.success)setBatches(d.data||[]);}
  ...
  function selectAll(){selected.size===batches.length?setSelected(new Set()):setSelected(new Set(batches.map(b=>b.id)));}

  async function deleteSelected(){
    setDeleting(true);
    try{
      const d=await apiPost({action:"deleteCardBatches",ids:Array.from(selected),token:admin.token});
      if(d.success){setMsg(`✅ ${selected.size} batch(es) deleted.`);setSelected(new Set());setDeleteMode(false);setConfirmDelete(false);loadBatches();}
      else setMsg("❌ "+d.message);
    }catch{setMsg("❌ Network error.");}finally{setDeleting(false);}
  }

  const typeColor={quiz:"#00f5ff",olympiad:"#a855f7",science_project:"#f59e0b"};

  return (
    <div className="space-y-6 max-w-3xl">
      <div className="glass-card p-6">
        <h3 className="font-bold text-white mb-1">🎴 Generate Registration Cards</h3>
        <p className="text-gray-500 text-xs mb-4">8 cards per A4 page. One card = one registration. Astro Photography is FREE — no cards needed.</p>
            ...
            <div className="flex flex-col gap-1">
              {CARD_TYPES.map(t=>(
                <button key={t.id} onClick={()=>setType(t.id)}
                  ...
                  style={{background:type===t.id?typeColor[t.id]+"22":"transparent",
                    color:type===t.id?typeColor[t.id]:"#9ca3af",
                    borderColor:type===t.id?typeColor[t.id]+"66":"rgba(255,255,255,0.1)"}}>
                  {t.id==="quiz"?"🧩":t.id==="olympiad"?"🔭":"🔬"} {t.label}
                </button>
              ))}
            </div>
          </div>
        ...
        <button onClick={generate} disabled={loading}
          ...
          style={{background:typeColor[type]||"#00f5ff",color:"#000",opacity:loading?0.6:1}}>
          {loading?<Spinner/>:`⚡ Generate ${count} ${CARD_TYPES.find(t=>t.id===type)?.label} Cards`}
        </button>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <h3 className="font-bold text-white">📦 All Generated Batches <span className="text-gray-500 font-normal text-sm">({batches.length})</span></h3>
          <div className="flex gap-2 flex-wrap items-center">
            ...
            {deleteMode&&(
              <div className="flex gap-2 items-center flex-wrap">
                <button onClick={selectAll} className="px-3 py-1.5 rounded-lg text-xs border border-white/20 text-gray-300 hover:border-white/40">
                  {selected.size===batches.length?"Deselect All":"Select All"}
                </button>
                {selected.size>0&&!confirmDelete&&(
                  <button onClick={()=>setConfirmDelete(true)}
                    className="px-3 py-1.5 rounded-lg text-xs font-bold border border-red-500/50 text-red-400 hover:bg-red-500/10">
                ...
                {confirmDelete&&(
                  <div className="flex gap-2 items-center">
                    ...
                    <button onClick={deleteSelected} disabled={deleting}
                      className="px-3 py-1.5 rounded-lg text-xs font-bold bg-red-500 text-white hover:bg-red-400"
                      style={{opacity:deleting?0.6:1}}>
                      {deleting?<Spinner sm/>:"✅ Confirm Delete"}
                    </button>
                    <button onClick={()=>setConfirmDelete(false)} className="px-3 py-1.5 rounded-lg text-xs border border-white/20 text-gray-400">Cancel</button>
                  </div>
                )}
                <button onClick={()=>{setDeleteMode(false);setSelected(new Set());setConfirmDelete(false);}}
                  className="px-3 py-1.5 rounded-lg text-xs border border-white/20 text-gray-400">✕ Exit</button>
              </div>
            )}
          </div>
        </div>
        ...
        {!fetching&&batches.map(batch=>(
          <div key={batch.id} className="glass-card p-4 transition-all"
            style={{borderColor:deleteMode&&selected.has(batch.id)?"rgba(239,68,68,0.6)":"rgba(255,255,255,0.08)",
            ...
            <div className="flex items-center gap-3 mb-3 flex-wrap">
              {deleteMode&&(
                <button onClick={()=>toggleSelect(batch.id)}
                  ...
                  style={{borderColor:selected.has(batch.id)?"#ef4444":"rgba(255,255,255,0.3)",background:selected.has(batch.id)?"#ef4444":"transparent"}}>
                  {selected.has(batch.id)&&<span className="text-white text-xs font-bold">✓</span>}
                </button>
              )}
              <div className="flex-1 flex items-center gap-2 flex-wrap">
                <span className="text-xs font-bold px-2 py-0.5 rounded-full"
                  style={{color:typeColor[batch.type]||"#00f5ff",background:(typeColor[batch.type]||"#00f5ff")+"18"}}>
                  {(batch.type||"quiz").replace("_"," ").toUpperCase()}
                </span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 max-h-32 overflow-y-auto">
              {(batch.cards||[]).slice(0,8).map((c,i)=>(
                <div key={i} className="rounded px-2 py-1.5 font-mono text-xs text-center border border-white/10"
                  style={{background:"#0f172a",color:typeColor[batch.type]||"#00f5ff"}}>{c.card_code||c}</div>
              ))}
              {(batch.cards||[]).length>8&&<div className="rounded px-2 py-1.5 text-xs text-center text-gray-500 border border-white/10"
...
function Registrations() {
  ...
  const isAstro = compType==="astro_photo";

  const q=search.toLowerCase();
  const filtered=(data||[]).filter(r=>{
    if(!q)return true;
    return[r.team_name,r.name,r.institution,r.contact_mobile,r.email,r.reg_id,
      r.bkash_txn_id,r.bkash_number,r.card_code,r.project_name,r.photo_name]
      .some(v=>(v||"").toString().toLowerCase().includes(q));
  });

  const payFiltered = isAstro ? filtered : filtered.filter(r=>{
    return payType==="card"?(r.payment_method==="card"||(r.card_code&&!r.bkash_txn_id)):(r.payment_method==="bkash"||r.bkash_txn_id);
  });

  const statusColor={confirmed:"#22c55e",pending:"#f59e0b",rejected:"#ef4444",registered:"#06b6d4"};
  ...
  return (
    <div className="space-y-4 max-w-4xl">
      ...
      <div className="flex gap-2 flex-wrap items-center">
        {COMP_TYPES.map(t=>(
          <button key={t.id} onClick={()=>setCompType(t.id)}
            ...
            style={{borderColor:compType===t.id?t.color:"rgba(255,255,255,0.1)",
        ...
        {data&&data.length>0&&(
          <button onClick={()=>generateRegListPDF(data,`${COMP_TYPES.find(t=>t.id===compType)?.label} Registrations`)}
      ...
      {/* Pay type tabs — only for non-astro */}
      {!isAstro&&(
      ...
      <SearchBar value={search} onChange={setSearch} placeholder="Search name, mobile, email, school, reg ID, project name..."/>
      ...
      {!loading&&data&&(
        <>
          <p className="text-gray-400 text-sm">
            <span className="text-white font-bold">{payFiltered.length}</span> of {data.length} registrations
            <button onClick={load} className="ml-3 text-cyan-400 hover:underline text-xs">🔄 Refresh</button>
          </p>
          {payFiltered.length===0?<div className="glass-card p-8 text-center text-gray-500">No registrations found.</div>
            :<div className="space-y-3">
              {payFiltered.map((r,i)=>{
                  ...
                  <div key={i} className="glass-card p-4">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        {compType==="science_project"&&<>
                          <p className="text-xs mt-0.5" style={{color:"#f59e0b"}}>🔬 {r.project_name}</p>
                          <p className="text-gray-500 text-xs">👥 {[r.member1_name,r.member2_name,r.member3_name].filter(Boolean).join(" · ")}</p>
                        </>}
                        {compType==="astro_photo"&&<>
                          <p className="text-xs mt-0.5" style={{color:"#06b6d4"}}>📸 {r.photo_name}</p>
                          {r.photo_link&&(
                            <a href={r.photo_link} target="_blank" rel="noreferrer"
                              className="text-xs underline mt-0.5 block hover:opacity-80 transition-opacity"
                              style={{color:"#06b6d4"}}>
                      ...
                      <div className="text-right space-y-1 shrink-0">
                        <p className="text-sm font-bold" style={{color:statusColor[r.payment_status||"pending"]}}>
                          ● {(r.payment_status||"pending").toUpperCase()}
                        </p>
                        {!isAstro&&payType==="bkash"&&<>
                          <p className="text-gray-400 text-xs">TxID: <span className="text-white">{r.bkash_txn_id}</span></p>
                          <p className="text-gray-400 text-xs">From: <span className="text-white">{r.bkash_number}</span></p>
                        </>}
                        {!isAstro&&payType==="card"&&<p className="text-gray-400 text-xs">Card: <span className="text-cyan-400 font-mono text-xs">{r.card_code}</span></p>}
...
function MarksEntry() {
  ...
  async function loadRegs(){
    ...
    try{
      const d=await apiPost({action:"getRegistrations",type:compType,token:admin.token});
      if(d.success){
        let confirmed=d.data.filter(r=>r.payment_status==="confirmed"||compType==="astro_photo");
        if(compType==="olympiad") confirmed=confirmed.filter(r=>(r.subjects||"").toLowerCase().includes(olympSubject.replace(/_/g," ").toLowerCase())||(r.subjects||"").toLowerCase().includes(olympSubject.toLowerCase()));
        setAllRegs(confirmed);
      }else setMsg("❌ "+d.message);
    }catch{setMsg("❌ Network error.");}finally{setLoading(false);}
  }
  ...
  async function loadMarks(){
    try{
      ...
      if(d.success&&Array.isArray(d.data)){
        const m={};d.data.forEach(r=>{m[r.reg_id]=r.marks||r.total_marks||0;});
  ...
  async function loadQualified(){
    try{const d=await apiPost({action:"getQualified",token:admin.token});if(d.success)setQualifiedIds(d.data||{});}
    catch{}
  }

  useEffect(()=>{loadRegs();if(compType==="quiz")loadQualified();},[compType,olympSubject]);
  useEffect(()=>{if(allRegs.length>0)loadMarks();},[quizRound,allRegs.length]);

  function getVisibleRegs(){
    if(compType==="quiz"){
      if(quizRound==="round1")return allRegs;
      const ids=qualifiedIds[quizRound]||[];
      return ids.length===0?[]:allRegs.filter(r=>ids.includes(r.reg_id));
    }
    return allRegs;
  }
    ...
    try{
      const payload=compType==="quiz"
        ...
        :compType==="olympiad"
          ...
          :compType==="science_project"
            ?{action:"saveMarks",type:"science_project",token:admin.token,reg_id,team_name:teamName,marks:val}
            :{action:"saveMarks",type:"astro_photo",token:admin.token,reg_id,name,marks:val};
  ...
  async function handleQualify(fromRound){
    ...
    try{
      ...
      const sorted=[...(d.data||[])].sort((a,b)=>(b.marks||0)-(a.marks||0));
      ...
      setMsg(`✅ Top ${n} teams qualified for ${ROUNDS.find(r=>r.id===nextRound)?.label}`);
    }catch{setMsg("❌ Network error.");}finally{setQualifying(false);}
  }

  const visible=getVisibleRegs();
  const q=search.toLowerCase();
  const filtered=visible.filter(r=>!q||[r.team_name,r.name,r.institution,r.reg_id].some(v=>(v||"").toString().toLowerCase().includes(q)));
  const ct=COMP_TYPES.find(t=>t.id===compType);
  ...
  return (
    <div className="space-y-4 max-w-4xl">
      ...
      <div className="flex gap-2 flex-wrap">
        {COMP_TYPES.map(t=>(
          <button key={t.id} onClick={()=>{setCompType(t.id);setSearch("");setSavedMarks({});setLocalMarks({});}}
            className="px-4 py-2 rounded-lg text-sm font-semibold border transition-all"
            style={{borderColor:compType===t.id?t.color:"rgba(255,255,255,0.1)",
              color:compType===t.id?t.color:"#9ca3af",
              background:compType===t.id?t.color+"18":"transparent"}}>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {ROUNDS.map(r=>{
                ...
                return(
                  <button key={r.id}
                    onClick={()=>{if(!isLocked){setQuizRound(r.id);setSearch("");setSavedMarks({});setLocalMarks({});}}}
          ...
          {quizRound!=="final"&&(
            <div className="border-t border-white/10 pt-4">
              ...
              <div className="flex gap-3 items-end mt-2">
                <div className="w-40">
                  <label className="block text-xs text-gray-500 mb-1">Qualifiers count</label>
                ...
                <button onClick={()=>handleQualify(quizRound)} disabled={qualifying}
                  className="px-5 py-2 rounded-lg text-sm font-bold border border-yellow-500/40 text-yellow-400 hover:bg-yellow-500/10"
          ...
          <div className="px-4 py-3 border-b border-white/10 flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-400">
                ...
                <span className="ml-2" style={{color:ct?.color}}>{ct?.icon} {ct?.label}</span>
          ...
          {filtered.length===0?(
            ...
            <div className="divide-y divide-white/5">
              {filtered.map((r,i)=>{
                ...
                const isDirty=String(localMarks[r.reg_id])!==String(savedMarks[r.reg_id]);
                return(
...
function Results() {
  ...
  useEffect(()=>{
    async function loadPublished(){
      try{
        ...
        if(d.success){const keys={};Object.keys(d.data).forEach(k=>{if(k.startsWith("result_"))keys[k]=d.data[k];});setPublishedKeys(keys);}
      }catch{}
    }
    loadPublished();
  },[]);
  ...
  async function load(){
    setLoading(true);setData(null);setMsg("");setSearch("");
    try{
      const payload={action:"getResults",type:compType,token:admin.token};
      if(compType==="quiz")payload.round=quizRound;
      if(compType==="olympiad")payload.subject=olympSubject;
      const d=await apiPost(payload);
      if(d.success){
  ...
  async function unpublish(){
    ...
    try{
      ...
      if(d.success){setMsg("✅ Result removed.");setPublishedKeys(p=>({...p,[publishKey]:""}));fetchSettings();}
      else setMsg("❌ "+d.message);
  ...
  const resultTitle=compType==="quiz"?`Quiz — ${roundLabel}`:compType==="olympiad"?`Olympiad — ${subjectLabel}`:ct?.label||"";
  ...
  return (
    <div className="space-y-4 max-w-4xl">
      ...
      <div className="flex gap-2 flex-wrap">
        {COMP_TYPES.map(t=>(
          <button key={t.id} onClick={()=>{setCompType(t.id);setData(null);}}
            className="px-4 py-2 rounded-lg text-sm font-semibold border transition-all"
            style={{borderColor:compType===t.id?t.color:"rgba(255,255,255,0.1)",
              color:compType===t.id?t.color:"#9ca3af",
              background:compType===t.id?t.color+"18":"transparent"}}>
            {t.icon} {t.label}
            {publishedKeys[`result_${t.id}`]==="published"&&t.id!=="quiz"&&t.id!=="olympiad"&&(
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <button onClick={load} disabled={loading}
          className="px-5 py-2.5 rounded-lg font-bold text-sm border border-cyan-400/40 text-cyan-400 hover:bg-cyan-400/10"
          style={{opacity:loading?0.6:1}}>
          {loading?<Spinner/>:"📊 Load Results"}
        </button>
        {data&&data.length>0&&(
          <button onClick={()=>generateResultPDF(filtered.length>0?filtered:data,resultTitle)}
            className="px-5 py-2.5 rounded-lg font-bold text-sm border border-blue-500/40 text-blue-400 hover:bg-blue-500/10">
            ⬇ Download PDF
          </button>
        )}
        {data&&data.length>0&&!isPublished&&(
          <button onClick={publish} disabled={publishing}
            className="px-5 py-2.5 rounded-lg font-bold text-sm border border-green-500/40 text-green-400 hover:bg-green-500/10"
        ...
        {isPublished&&(
          <div className="flex items-center gap-3">
            <span className="text-green-400 text-sm font-semibold flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse inline-block"/>Published
            </span>
            <button onClick={unpublish} disabled={removing}
              className="px-4 py-2 rounded-lg text-xs font-bold border border-red-500/30 text-red-400 hover:bg-red-500/10"
        ...
        <div className="glass-card overflow-hidden">
          ...
          {filtered.length===0?<div className="p-8 text-center text-gray-500">No marks entered yet.</div>
            :<table className="w-full text-sm">
              <thead style={{background:`rgba(${compType==="quiz"?"0,245,255":compType==="olympiad"?"168,85,247":compType==="science_project"?"245,158,11":"6,182,212"},0.08)`}}>
                <tr>
                  <th className="px-4 py-3 text-left font-semibold w-16" style={{color:ct?.color}}>Rank</th>
                  <th className="px-4 py-3 text-left font-semibold" style={{color:ct?.color}}>Name / Team</th>
                  <th className="px-4 py-3 text-left font-semibold hidden md:table-cell" style={{color:ct?.color}}>Institution</th>
                  <th className="px-4 py-3 text-right font-semibold" style={{color:ct?.color}}>Marks</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((r,i)=>(
                  <tr key={i} className="border-t border-white/5" style={{background:i<3?"rgba(234,179,8,0.04)":"transparent"}}>
                    ...
                    <td className="px-4 py-3 text-right font-bold" style={{color:ct?.color}}>{r.total_marks||r.marks||0}</td>
                  </tr>
                ))}
              </tbody>
            </table>}
    ...
    <div className="max-w-2xl">
      <form onSubmit={handleSave} className="glass-card p-6 space-y-5">
        <div className="border-b border-white/10 pb-3">
          <h3 className="font-bold text-cyan-400">🖼️ Logo & Banner</h3>
        ...
        <F label="Logo URL" k="logo_url" placeholder="https://i.ibb.co/xxxxx/logo.png"
          hint="ImgBB → Upload → Copy 'Direct Link'"/>
        ...
        <F label="Countdown Date" k="countdown_date" type2="date"/>
        {form.countdown_date&&<p className="text-cyan-400 text-xs -mt-3">📅 {new Date(form.countdown_date).toLocaleDateString("en-GB",{weekday:"long",day:"numeric",month:"long",year:"numeric"})}</p>}
        ...
        {msg&&<div className="p-4 rounded-lg text-sm border"
          style={{color:msg.includes("✅")?"#4ade80":"#f87171",borderColor:msg.includes("✅")?"rgba(74,222,128,0.2)":"rgba(248,113,113,0.2)",background:msg.includes("✅")?"rgba(74,222,128,0.08)":"rgba(248,113,113,0.08)"}}>
          {msg}</div>}
        <button type="submit" disabled={loading} className="w-full py-3 rounded-lg font-bold text-lg"
