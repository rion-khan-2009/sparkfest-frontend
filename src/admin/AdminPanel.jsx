  ...
  while (idx < cards.length) {
    ...
    for (let row = 0; row < 4; row++) {
      for (let col = 0; col < 2; col++) {
        ...
        doc.setDrawColor(0, 150, 180); doc.setLineWidth(0.5);
        doc.roundedRect(x, y, cardW, cardH, 3, 3, "S");
        doc.setLineDashPattern([1, 1], 0); doc.setDrawColor(200, 200, 200);
        doc.roundedRect(x + 0.5, y + 0.5, cardW - 1, cardH - 1, 3, 3, "S");
        ...
        doc.line(x + 5, y + 27, x + cardW - 5, y + 27);
        ...
        doc.text(siteUrl || "sparkfest.netlify.app", x + cardW / 2, y + 37, { align: "center" });
...
async function generateRegListPDF(data, title) {
  ...
  doc.text("#", 14, y); doc.text("Name / Team", 24, y);
  doc.text("Institution", 90, y); doc.text("Status", 170, y);
  y += 8;
  data.forEach((r, i) => {
...
// ─── PDF: Result — WHITE background, no emoji ────────────────
async function generateResultPDF(data, title) {
  ...
  // Thin accent line
  doc.setDrawColor(0, 200, 220); doc.setLineWidth(0.5);
  doc.line(15, 33, pageW - 15, 33);
  // Top 3 — light colored boxes, dark text, NO emoji
  let y = 42;
  ...
  data.slice(0, 3).forEach((r, i) => {
    ...
    doc.setDrawColor(...borderColors[i]); doc.setLineWidth(0.4);
    doc.roundedRect(10, y, pageW - 20, 14, 2, 2, "S");
    doc.setFont("helvetica", "bold"); doc.setFontSize(8.5);
    doc.setTextColor(...rankColors[i]);
    doc.text(medals[i], 18, y + 9);
    doc.setTextColor(20, 20, 20);
    doc.text((r.team_name || r.name || "").substring(0, 34), 55, y + 9);
    doc.setTextColor(0, 80, 160);
    doc.text(String(r.total_marks || r.marks || 0), pageW - 18, y + 9, { align: "right" });
    y += 16;
  });
  y += 4;
  // Rest — plain table
  ...
  doc.text("Rank", 14, y); doc.text("Name / Team", 30, y);
  doc.text("Institution", 110, y); doc.text("Marks", pageW - 18, y, { align: "right" });
  y += 8;
  data.slice(3).forEach((r, i) => {
    ...
    doc.text((r.team_name || r.name || "").substring(0, 32), 30, y);
    doc.setTextColor(100, 100, 100);
    doc.text((r.institution || "").substring(0, 20), 110, y);
    doc.setFont("helvetica", "bold"); doc.setTextColor(0, 80, 160);
    doc.text(String(r.total_marks || r.marks || 0), pageW - 18, y, { align: "right" });
    y += 9;
  });
  doc.setFont("helvetica", "italic"); doc.setFontSize(7); doc.setTextColor(150, 150, 150);
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setStats({
          quiz: q.data?.length || 0, olympiad: o.data?.length || 0,
          qPending: q.data?.filter(r => r.payment_status === "pending").length || 0,
          oPending: o.data?.filter(r => r.payment_status === "pending").length || 0,
        });
      } catch { }
    }
    load();
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { icon: "🧩", label: "Quiz Regs",     value: stats?.quiz,     color: "#00f5ff" },
          { icon: "🔭", label: "Olympiad Regs", value: stats?.olympiad, color: "#a855f7" },
          { icon: "⏳", label: "Quiz Pending",   value: stats?.qPending, color: "#f59e0b" },
          { icon: "⏳", label: "Olym Pending",  value: stats?.oPending, color: "#f59e0b" },
        ].map((s, i) => (
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="glass-card p-5">
          ...
          <div className="space-y-2">
            {[{ label: "Generate Cards", tab: "cards", icon: "🎴" }, { label: "Registrations", tab: "regs", icon: "📋" },
              { label: "Exam Marks", tab: "marks", icon: "📝" }, { label: "Results", tab: "results", icon: "🏆" }].map(a => (
              <button key={a.tab} onClick={() => setTab(a.tab)}
                className="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm text-left border border-white/5 hover:border-cyan-400/30 hover:text-cyan-400 transition-all text-gray-300">
        ...
        <div className="glass-card p-5">
          <h3 className="font-bold text-white mb-3">📢 Notices ({notices.length})</h3>
          {notices.length === 0 ? <p className="text-gray-500 text-sm">No notices.</p>
            : notices.slice(0, 4).map((n, i) => (
              <div key={i} className="py-2 border-b border-white/5 last:border-0">
                <p className="text-white text-sm font-medium truncate">{n.title}</p>
                <p className="text-gray-500 text-xs">{new Date(n.created_at).toLocaleDateString()}</p>
              </div>
            ))}
        </div>
      </div>
...
function CardGenerator() {
  ...
  const [siteUrl, setSiteUrl] = useState("sparkfest.netlify.app");

  // Load batches from BACKEND (shared across all admins)
  ...
  async function generate() {
    ...
    try {
      ...
      if (d.success) {
        setMsg(`✅ ${d.data.length} ${type} cards generated!`);
        loadBatches(); // reload from backend
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="glass-card p-6">
        ...
        <p className="text-gray-500 text-xs mb-4">8 cards per A4 page. One card = one registration only. All admins share the same card list.</p>
        <div className="grid sm:grid-cols-3 gap-4 mb-4">
          ...
          <div><label className="block text-sm text-gray-400 mb-1">Count</label>
            <input type="number" min="1" max="200" className="input-dark" value={count} onChange={e=>setCount(e.target.value)}/></div>
          <div><label className="block text-sm text-gray-400 mb-1">Website URL (on card)</label>
            <input type="text" className="input-dark" placeholder="sparkfest.netlify.app" value={siteUrl} onChange={e=>setSiteUrl(e.target.value)}/></div>
        </div>
      ...
      {/* Shared batches from backend */}
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-white">📦 All Generated Batches</h3>
          <button onClick={loadBatches} className="text-cyan-400 text-xs hover:underline">🔄 Refresh</button>
        </div>
        ...
        {!fetching && batches.map(batch=>(
          <div key={batch.id} className="glass-card p-4">
            <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
              <div>
                <span className={`text-xs font-bold px-2 py-0.5 rounded-full mr-2 ${batch.type==="quiz"?"text-cyan-400 bg-cyan-400/10":"text-purple-400 bg-purple-400/10"}`}>
                  {(batch.type||"quiz").toUpperCase()}</span>
                <span className="text-white font-semibold">{batch.count} Cards</span>
                <span className="text-gray-500 text-xs ml-2">{new Date(batch.createdAt).toLocaleString()}</span>
              </div>
              <button onClick={()=>generateCardsPDF(batch.cards, batch.type||"quiz", siteUrl)}
                className="px-3 py-1.5 rounded-lg text-xs font-bold border border-cyan-400/40 text-cyan-400 hover:bg-cyan-400/10 transition-all">
                ⬇ Download PDF
              </button>
            </div>
  ...
  return (
    <div className="space-y-4 max-w-4xl">
      <div className="flex gap-2 flex-wrap items-center">
        {["quiz", "olympiad"].map(t => (
          <button key={t} onClick={() => setCompType(t)}
            ...
            style={{ borderColor: compType === t ? (t === "quiz" ? "#00f5ff" : "#a855f7") : "rgba(255,255,255,0.1)", color: compType === t ? (t === "quiz" ? "#00f5ff" : "#a855f7") : "#9ca3af", background: compType === t ? (t === "quiz" ? "rgba(0,245,255,0.1)" : "rgba(168,85,247,0.1)") : "transparent" }}>
      ...
      <div className="flex gap-2 p-1 rounded-xl border border-white/10 w-fit" style={{ background: "#111827" }}>
        {["bkash", "card"].map(p => (
          <button key={p} onClick={() => setPayType(p)} className="px-4 py-2 rounded-lg text-sm font-semibold capitalize transition-all"
...
// ── MARKS ENTRY ───────────────────────────────────────────────
// Marks are saved in localStorage AND backend
// Quiz rounds are sequential: only qualified teams move to next round
function MarksEntry() {
  const { admin } = useAuth();
  const [compType, setCompType] = useState("quiz");
  const [quizRound, setQuizRound] = useState("round1");
  const [olympSubject, setOlympSubject] = useState("mathematics");
  const [allRegs, setAllRegs] = useState([]); // all confirmed registrations
  const [savedMarks, setSavedMarks] = useState({}); // { reg_id: marks } from backend
  const [localMarks, setLocalMarks] = useState({}); // currently editing
  const [qualifiedIds, setQualifiedIds] = useState({}); // { round2: [ids], round3: [ids], final: [ids] }
  ...
  // Load saved marks from backend for current view
  async function loadMarks() {
    try {
      ...
      if (d.success && Array.isArray(d.data)) {
        ...
        setSavedMarks(m);
        setLocalMarks(m); // pre-fill local with saved
      }
    } catch { }
  }

  // Load qualified IDs from BACKEND (shared across all admins)
  ...
  // Who to show based on round
  function getVisibleRegs() {
    if (compType === "olympiad") {
      return allRegs.filter(r => (r.subjects || "").toLowerCase().includes(olympSubject.replace(/_/g, " ").toLowerCase()) ||
        (r.subjects || "").toLowerCase().includes(olympSubject.toLowerCase()));
    }
    // Quiz round logic
    if (quizRound === "round1") return allRegs;
    const key = quizRound; // round2, round3, final
    const ids = qualifiedIds[key] || [];
  ...
  async function saveMark(reg_id, name, teamName) {
    ...
    try {
      ...
      if (d.success) {
        setSavedMarks(p => ({ ...p, [reg_id]: val }));
        setMsg("✅ Marks saved for " + (teamName || name));
      } else setMsg("❌ " + d.message);
  ...
  async function handleQualify(fromRound) {
    ...
    try {
      ...
      // Save to BACKEND so all admins see the same qualified list
    ...
    <div className="space-y-4 max-w-4xl">
      ...
      {compType === "quiz" && (
        <div className="glass-card p-4 space-y-4">
              ...
              {ROUNDS.map(r => {
                ...
                return (
                  <button key={r.id} onClick={() => { if (!isLocked) { setQuizRound(r.id); setSearch(""); setSavedMarks({}); setLocalMarks({}); } }}
                    ...
                    style={{ borderColor: quizRound === r.id ? "#00f5ff" : isLocked ? "rgba(255,255,255,0.05)" : "rgba(255,255,255,0.15)", color: quizRound === r.id ? "#00f5ff" : isLocked ? "#374151" : "#9ca3af", background: quizRound === r.id ? "rgba(0,245,255,0.1)" : "transparent", cursor: isLocked ? "not-allowed" : "pointer" }}>
                    {r.label}
                    {isLocked && <span className="ml-1 text-gray-600">🔒</span>}
                    {!isLocked && r.id !== "round1" && qualifiedIds[r.id] && (
                      <span className="ml-1 text-xs text-cyan-400">({qualifiedIds[r.id].length})</span>
                    )}
                  </button>
                );
              })}
          ...
          {/* Qualify section — shown for round1, round2, round3 */}
          {quizRound !== "final" && (
            <div className="border-t border-white/10 pt-4">
              <p className="text-sm text-gray-300 font-semibold mb-1">
                🏅 Qualify teams: {ROUNDS.find(r => r.id === quizRound)?.label} → {ROUNDS.find(r => r.id === quizRound)?.next ? ROUNDS.find(r => r.id === ROUNDS.find(x => x.id === quizRound)?.next)?.label : ""}
              </p>
              <p className="text-gray-500 text-xs mb-3">Enter how many top teams advance. Marks must be saved first.</p>
              <div className="flex gap-3 items-end">
                <div className="w-40">
                  <label className="block text-xs text-gray-500 mb-1">Number of qualifiers</label>
                  <input type="number" min="1" className="input-dark text-sm py-2"
                    placeholder="e.g. 10"
                    value={qualifyInput[quizRound] || ""}
                    onChange={e => setQualifyInput(p => ({ ...p, [quizRound]: e.target.value }))} />
                </div>
          ...
          <div className="flex flex-wrap gap-2">
            {SUBJECTS.map(s => (
              <button key={s.id} onClick={() => { setOlympSubject(s.id); setSearch(""); setSavedMarks({}); setLocalMarks({}); }}
                ...
                style={{ borderColor: olympSubject === s.id ? "#a855f7" : "rgba(255,255,255,0.1)", color: olympSubject === s.id ? "#a855f7" : "#9ca3af", background: olympSubject === s.id ? "rgba(168,85,247,0.1)" : "transparent" }}>
                {s.label}
              </button>
            ))}
          </div>
        ...
        <div className="glass-card overflow-hidden">
          <div className="px-4 py-3 border-b border-white/10 flex items-center justify-between">
            <div>
              ...
              {compType === "quiz" && quizRound !== "round1" && visible.length === 0 && (
                <p className="text-yellow-400 text-xs mt-0.5">⚠️ No teams qualified for this round yet. Qualify from previous round first.</p>
              )}
            </div>
          ...
          {filtered.length === 0 ? (
            <div className="p-8 text-center text-gray-500">
              {compType === "quiz" && quizRound !== "round1"
                ? "No teams qualified for this round yet."
              ...
              {filtered.map((r, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-3 px-4 py-3 hover:bg-white/2">
                    ...
                    {isSaved && !isDirty && (
                      <span className="text-green-400 text-xs shrink-0">✓ Saved: {savedMarks[r.reg_id]}</span>
                    )}
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button onClick={() => saveMark(r.reg_id, r.name, r.team_name)}
                        disabled={saving[r.reg_id]}
                        className="px-3 py-1.5 rounded-lg text-xs font-bold border transition-all whitespace-nowrap"
                        style={{ borderColor: isDirty ? "#00f5ff" : "rgba(255,255,255,0.1)", color: isDirty ? "#00f5ff" : "#6b7280", background: isDirty ? "rgba(0,245,255,0.08)" : "transparent", opacity: saving[r.reg_id] ? 0.6 : 1 }}>
...
function Results() {
  ...
  useEffect(() => {
    // Load published state from settings
        ...
        <div className="flex flex-wrap gap-2">
          {ROUNDS.map(r => (
            <button key={r.id} onClick={() => { setQuizRound(r.id); setData(null); }}
              className="px-3 py-1.5 rounded-lg text-sm border transition-all"
              style={{ borderColor: quizRound === r.id ? "#00f5ff" : "rgba(255,255,255,0.1)", color: quizRound === r.id ? "#00f5ff" : "#9ca3af", background: quizRound === r.id ? "rgba(0,245,255,0.1)" : "transparent" }}>
              {r.label}
              {publishedKeys[`result_quiz_${r.id}`] === "published" && <span className="ml-1 text-green-400 text-xs">●</span>}
            </button>
          ))}
        </div>
      )}
        ...
        <div className="flex flex-wrap gap-2">
          {SUBJECTS.map(s => (
            <button key={s.id} onClick={() => { setOlympSubject(s.id); setData(null); }}
              ...
              style={{ borderColor: olympSubject === s.id ? "#a855f7" : "rgba(255,255,255,0.1)", color: olympSubject === s.id ? "#a855f7" : "#9ca3af", background: olympSubject === s.id ? "rgba(168,85,247,0.1)" : "transparent" }}>
              {s.label}
              {publishedKeys[`result_olympiad_${s.id}`] === "published" && <span className="ml-1 text-green-400 text-xs">●</span>}
            </button>
          ))}
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        {isPublished && (
          <div className="flex items-center gap-3">
            <span className="text-green-400 text-sm font-semibold">● Published on website</span>
      ...
      {data && (
        <div className="glass-card overflow-hidden">
          <div className="px-4 py-3 border-b border-white/10">
            <p className="text-sm font-semibold text-white">{resultTitle}
              <span className="text-gray-500 ml-2">({filtered.length} entries)</span>
            </p>
          </div>
...
function Settings() {
  ...
  useEffect(() => {
    if (!loaded && settings.event_title) {
      setForm({ logo_url: settings.logo_url || "", banner_image_url: settings.banner_image_url || "", event_title: settings.event_title || "", countdown_date: settings.countdown_date || "", facebook_url: settings.facebook_url || "" });
      setLoaded(true);
    }
  }, [settings]);
  ...
  return (
    <div className="max-w-2xl">
      <form onSubmit={handleSave} className="glass-card p-6 space-y-5">
        <div className="border-b border-white/10 pb-3"><h3 className="font-bold text-cyan-400">🖼️ Logo & Banner</h3></div>
        <F label="Logo URL" k="logo_url" placeholder="https://i.ibb.co/logo.png" hint="Upload to ImgBB, paste direct link" />
        {form.logo_url && <img src={form.logo_url} alt="Logo" className="h-16 w-16 object-contain rounded-lg border border-white/10" />}
        <F label="Banner Image URL" k="banner_image_url" placeholder="https://i.ibb.co/banner.jpg" hint="1920×600px recommended" />
        ...
        {msg && <div className="p-4 rounded-lg text-sm border" style={{ color: msg.includes("✅") ? "#4ade80" : "#f87171", borderColor: msg.includes("✅") ? "rgba(74,222,128,0.2)" : "rgba(248,113,113,0.2)", background: msg.includes("✅") ? "rgba(74,222,128,0.08)" : "rgba(248,113,113,0.08)" }}>{msg}</div>}
